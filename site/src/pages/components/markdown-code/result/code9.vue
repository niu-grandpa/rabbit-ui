<template>
    <div class="markdown-code">
        <div class="code-write">
            <p style="font-size: 32px">Attributes</p>
            <h3><a name="result" class="md-header-anchor"></a><span>Result</span></h3>
            <figure>
                <table>
                    <thead>
                        <tr>
                            <th><span>属性</span></th>
                            <th><span>说明</span></th>
                            <th><span>默认值</span></th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td><span>status</span></td>
                            <td>
                                <span>结果的状态，决定图标和颜色。可选值为 </span
                                ><code>success</code><span>、</span><code>error</code><span>、</span
                                ><code>info</code><span> 、 </span><code>warning</code
                                ><span> 、 </span><code>404</code><span>、</span><code>403</code
                                ><span>、 </span><code>500</code>
                            </td>
                            <td><span>info</span></td>
                        </tr>
                        <tr>
                            <td><span>title</span></td>
                            <td><span>标题文字</span></td>
                            <td><span>-</span></td>
                        </tr>
                        <tr>
                            <td><span>subtitle</span></td>
                            <td><span>结果描述</span></td>
                            <td><span>-</span></td>
                        </tr>
                        <tr>
                            <td><span>extra</span></td>
                            <td><span>操作区，建议放置按钮组</span></td>
                            <td><span>-</span></td>
                        </tr>
                        <tr>
                            <td><span>icon</span></td>
                            <td><span>自定义图标</span></td>
                            <td><span>-</span></td>
                        </tr>
                    </tbody>
                </table>
            </figure>
        </div>
    </div>
</template>